...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['edit_product'])) {
    ...
    $image_paths = json_decode($_POST['existing_images'], true) ?: [];
...
if (isset($_GET['delete'])) {
    ...
    $check_stmt = $conn->prepare("SELECT COUNT(*) as order_count FROM order_items WHERE product_id = ?");
    ...
    if ($check_data['order_count'] > 0) {
        $_SESSION['error_message'] = "Cannot delete this product. It is referenced in " . $check_data['order_count'] . " order(s). You can set it as inactive instead.";
        header("Location: products.php");
        exit();
    } else {
...
<body class="bg-gray-50">
                            ...
                            <div class="product-card border border-gray-200 rounded-lg overflow-hidden hover:shadow-md transition"
                                ...
                                <div class="p-4">
                                    ...
                                    <div class="flex gap-2">
                                        <button onclick='openEditModal(<?php echo json_encode($product); ?>)' class="flex-1 text-center bg-blue-100 text-blue-700 py-2 rounded text-sm hover:bg-blue-200 transition font-semibold">Edit</button>
                                        <button onclick='confirmDelete(<?php echo $product["id"]; ?>, "<?php echo htmlspecialchars($product["name"]); ?>")' class="flex-1 text-center bg-red-100 text-red-700 py-2 rounded text-sm hover:bg-red-200 transition font-semibold">Delete</button>
                                    </div>
                                </div>
                            </div>
                            <?php endwhile; ?>
    ...
    <div id="deleteModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 modal">
        <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 overflow-hidden">
            <div class="p-6 text-center">
                ...
                <p class="font-semibold text-gray-900 mb-6" id="deleteProductName"></p>
    ...
    <script>
        ...
        // Override delete links to use modal
