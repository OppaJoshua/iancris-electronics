    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
        ...
        if (in_array(strtolower($filetype), $allowed)) {
            ...
            if (move_uploaded_file($_FILES['image']['tmp_name'], $upload_path . $newname)) {
                ...
                $stmt = $conn->prepare("INSERT INTO gallery (title, description, location, image) VALUES (?, ?, ?, ?)");
                $stmt->bind_param("ssss", $title, $description, $location, $image_path);
    ...
    if (isset($_FILES['edit_image']) && $_FILES['edit_image']['error'] == 0) {
        ...
        if (in_array(strtolower($filetype), $allowed)) {
            ...
            if (move_uploaded_file($_FILES['edit_image']['tmp_name'], $upload_path . $newname)) {
                ...
                $stmt = $conn->prepare("UPDATE gallery SET title = ?, description = ?, location = ?, image = ? WHERE id = ?");
                $stmt->bind_param("ssssi", $title, $description, $location, $image_path, $id);
            }
        } else {
            $error = "Invalid file type. Only JPG, JPEG, PNG & GIF allowed.";
        }
        ...
        $stmt = $conn->prepare("UPDATE gallery SET title = ?, description = ?, location = ? WHERE id = ?");
