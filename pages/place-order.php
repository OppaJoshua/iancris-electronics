...
try {
    ...
    $total_amount = 0.00; // Set to 0 since products have no price
    
    error_log("place-order.php: Creating order - Name: $user_name, Phone: $user_phone, Items: $total_items");

    // Create order - including total_amount column
    $stmt = $conn->prepare("INSERT INTO orders (user_id, user_name, user_phone, user_address, total_items, total_amount, status) VALUES (?, ?, ?, ?, ?, ?, 'pending')");
    if (!$stmt) {
        error_log("place-order.php: Prepare order insert failed: " . $conn->error);
        throw new Exception("Database prepare error: " . $conn->error);
    }
    
    $stmt->bind_param("isssid", $user_id, $user_name, $user_phone, $user_address, $total_items, $total_amount);
